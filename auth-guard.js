...
/* Bloque "Sesión" al final de la página (si existe #sessionFooter),
   con el mismo estilo de tarjeta que el resto del contenido. */
function renderSessionFooter(user, auth){
  const footer = document.getElementById("sessionFooter");
  if (!footer) return;
  footer.innerHTML = `
    <a class="group-card" href="#" id="sessionLogoutCard" tabindex="0" role="button">
      <span class="session-email">${user.email}</span>
      <span class="group-count">Cerrar sesión</span>
    </a>
  `;
